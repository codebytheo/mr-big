...
export const Paragraph = ({text,cn}:props) => {
  return (
    <p className={`text-zinc-300 indent-20 text-lg font-intermedium text-justify ${cn}`}>{text}</p>
  )
}
...
export const TextTitle = ({text,cn}:props) => {
  ...
  return (
    <motion.h1 
      ref={container}
      style={{translateY:y}}
      className={`relative font-interextrabold text-center tracking-[-8px] leading-none text-softwhite ${cn}`}
      >
        {text}
    </motion.h1>
  )
